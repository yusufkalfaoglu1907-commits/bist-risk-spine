...
- **Hyperinflation accounting (TMS 29 / IAS 29).** From FY2023 Turkish issuers report **inflation-adjusted** financials. Revenue, COGS, and segment figures — which the exposure/materiality weights depend on — are not comparable across the restatement boundary. Carry a `restatement_regime` flag on every fundamental datum and never compute a growth/intensity figure that straddles the boundary without converting to a common basis.
...
- **Short-sale constraints.** Shorting on BIST is restricted and **periodically banned outright during stress**. This does not change the (long-short) signal design, but it bounds tradability exactly when signals are strongest; tracked in §10.
...
Every edge carries: `source`, `confidence ∈ [0,1]`, `valid_from`, `valid_to`, `knowledge_date`.
...
- Fit a **multi-factor model** per `universe_class` (market + FX + sector + commodity + **foreign-flow**), rolling and regime-tagged; take **residual returns**. The foreign-flow factor must be in the model or flow-driven comovement masquerades as residual linkage.
- Build the network on residuals, not raw returns. With ~500 names and limited clean history you are in the `p ≈ n` regime, so the sample covariance is **ill-conditioned** — apply **shrinkage (Ledoit–Wolf)** or impose factor structure before inverting anything.
...
- **Combiner + backtester:** strict point-in-time, liquidity-screened, net-of-cost. This harness is the asset; the edges are raw material.

---

## 10. Honest risks & limitations

1. **The best alpha needs the worst data.** Linked-firm predictability (your most novel edge) depends on firm-level supply-chain identities — the scarcest input [C]. Mitigation: lead with the correlation and event pillars (data-light), grow supply-chain coverage incrementally, and never let a tier-3 (sector-IO) edge masquerade as a firm-level one.
...
5. **Tradability illusion.** Many BIST names are thin. A signal that backtests beautifully on illiquid names is untradeable. Liquidity-screened, cost-net backtesting is the only honest test.
   - **Short-leg fragility (live).** You chose long-short. BIST short-selling is constrained and **periodically banned during stress** — the very regimes where event/stat-arb signals peak. The research book can stay long-short for clean alpha isolation, but model a **short-availability state** per name per date and report deployable (short-constrained) P&L alongside the ideal. If the edge survives only with unconstrained shorting, it is not real for you.
