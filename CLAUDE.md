...
- **Authoritative design:** `docs/system-design-v2.md` (v2.1). The hardening rationale is in `docs/v2.1-hardening-memo.md`. **Do not re-derive the design — implement it.** If you think the design is wrong, log it in `docs/BUILD_LOG.md` and raise it; don't silently deviate.
- **Data sourcing (already verified live):** `docs/data-sourcing-v2.md` + `docs/data-sourcing-matrix-v2.xlsx`. The verified spine is the **Matriks MCP**; supporting sources are EVDS, KAP, GDELT, borsa-mcp.
...
**v2 is the build ahead of you.** The current roadmap and gates are in `docs/BUILD_PLAN.md`. Always check it for the current milestone before starting.
...
These are the things that quietly corrupt every signal. They are engineered in from commit 1, never "flagged and ignored." Full spec in `docs/system-design-v2.md` §3/§6; condensed:
...
- **The id-bridge** (ticker ↔ ISIN ↔ mkkMemberOid ↔ LEI) is a single point of failure. It has its own resolution test (see `docs/VERIFICATION.md`).
...
- **Every behavior change ships with a test in the same change.** New invariant → new invariant test (see `docs/VERIFICATION.md`).
...
1. Read the latest entry in `docs/BUILD_LOG.md` and the current milestone in `docs/BUILD_PLAN.md`.
2. Run the verification suite (`docs/VERIFICATION.md` → "Run it"). If it's RED, your first job is to get it green or understand why before adding anything.
...
2. Append a `docs/BUILD_LOG.md` entry: what you attempted, what passed/failed, decisions made, open threads, and the **single next action**.
3. Update this file or `docs/BUILD_PLAN.md` **only** for durable invariants/decisions (not transient state). New consequential decision → new ADR.
...
- A **project-level go/no-go gate** is reached (e.g. the residual-survival gate in `docs/BUILD_PLAN.md` M3) — surface the result and decision, don't auto-proceed.
- You're about to deviate from `docs/system-design-v2.md`, delete data, or weaken a test to make it pass.
...
| `docs/system-design-v2.md` | Authoritative design (v2.1) | Don't edit during build; raise issues in BUILD_LOG |
| `docs/BUILD_PLAN.md` | Phased milestones + go/no-go gates | When a milestone completes or re-plans |
| `docs/VERIFICATION.md` | Standing QA protocol, invariant suite, definition-of-done | When a new invariant/gate is added |
| `docs/BUILD_LOG.md` | Append-only session journal (the handoff) | **Every session** |
| `decisions/ADR-*.md` | One consequential decision each, append-only | When a fork is decided |
| `docs/data-sourcing-v2.md` + `.xlsx` | Verified sources, walls, connectors | Reference |
